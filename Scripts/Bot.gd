...
func scan_bot(for_class:String, include_self := true) -> Array[Bit]:
	return _get_sub_bit(for_class, include_self)

## Get any bits in the children, using a class_name.
func _get_sub_bit(bit_class:String, include_self, depth := 4, with:Node = self) -> Array[Bit]:
	...
	if include_self and get_script().get_global_name() == bit_class:
		results.append(self)
	...
	for child in with.get_children():
		...
		if child is Bit:
			...
			if child.get_script().get_global_name() == bit_class and child.get_bot() == self:
